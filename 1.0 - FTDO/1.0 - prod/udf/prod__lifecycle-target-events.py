  ...
  if cohort == "free trial drop out":
    query = f"""
      ...
      braze as (
        ...
        where lower(canvas_name) like '%ftdropout%'
      ...
      all_users as (
        select s.hs_user_id,
               ...
               b.canvas_entry_date,
               s.voucher_code,
               lead(s.voucher_code) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_voucher_code,
               s.dt signup_date,
               ...
               s.created_timestamp,
               s.updated_timestamp,
               lead(s.free_trial_start_date) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_free_trial_start_date,
               lead(s.free_trial_end_date) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_free_trial_end_date,
               lead(s.paid_subscription_start_date) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_paid_subscription_start_date,
               lead(s.paid_subscription_end_date) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_paid_subscription_end_date,
               --lead(s.subscriber_status) over (partition by s.hs_user_id order by s.dt, s.created_timestamp, s.updated_timestamp) next_subscriber_status,
